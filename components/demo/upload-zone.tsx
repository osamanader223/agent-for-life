...
export function UploadZone({ onComplete }: UploadZoneProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        {loading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-lg font-bold text-brand">جاري المعالجة...</p>
            <p className="text-sm text-[#6b7280]">
              الذكاء الاصطناعي يقرأ الفاتورة
            </p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-3">
      ...
      {/* Camera option — shown only on narrow viewports */}
      <label
        htmlFor="camera-capture"
        className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-cream-dark bg-white py-3 text-sm font-semibold text-[#6b7280] transition-colors hover:border-brand hover:text-brand md:hidden"
      >
